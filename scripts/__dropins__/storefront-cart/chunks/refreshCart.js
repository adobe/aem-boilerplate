...
`,Cr=`
  ...
  ${mr}
  ${A}
`,D=async()=>{const r=t.authenticated,n=t.cartId;if(r)return U(Cr,{method:"POST"}).then(({errors:c,data:e})=>{if(c)return I(c);const u={...e.cart,...e.customer};return N(u)});if(!n)throw new Error("No cart ID found");return U(vr,{method:"POST",cache:"no-cache",variables:{cartId:n}}).then(({errors:c,data:e})=>c?I(c):N(e.cart))},hr=`
  ...
  ${A}
`,G=async()=>{if(t.initializing)return null;t.initializing=!0,t.config||(t.config=await xr());const r=t.authenticated?await W():await q();return f.emit("cart/initialized",r),f.emit("cart/data",r),t.initializing=!1,r};async function W(){const r=t.cartId,n=await D();return n?(t.cartId=n.id,!r||n.id===r?n:await U(hr,{variables:{guestCartId:r,customerCartId:n.id}}).then(()=>D()).then(c=>{const e={oldCartItems:n.items,newCart:c};return f.emit("cart/merged",e),c}).catch(()=>(console.error("Could not merge carts"),n))):null}async function q(){if(V.getConfig().disableGuestCart===!0||!t.cartId)return null;try{return await D()}catch(r){return console.error(r),null}}const br=`
...
`,xr=async()=>U(br,{method:"GET",cache:"force-cache"}).then(({errors:r,data:n})=>r?I(r):dr(n.storeConfig)),Tr=async()=>{const r=t.authenticated?await W():await q();return f.emit("cart/data",r),r};export{R as C,G as a,W as b,V as c,q as d,xr as e,D as g,B as i,Tr as r,N as t};
