...
/**
 * Initializes and decorates the Purchase Order Comments List block
 * Redirects unauthenticated users and handles permission updates
 */
const renderPurchaseOrderCommentsList = async (blockElement, permissions = {}) => {
...
export default async function decorate(block) {
  ...
  await renderPurchaseOrderCommentsList(block, initialPermissions);

  // React to permission updates
  events.on('auth/permissions', async (updatedPermissions) => {
    await renderPurchaseOrderCommentsList(block, updatedPermissions);
  });
