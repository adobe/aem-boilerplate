...
/**
 * Initializes and decorates the Purchase Order Status block
...
const renderPurchaseOrderStatus = async (
  ...
  await purchaseOrderRenderer.render(PurchaseOrderStatus, {})(blockElement);
};
...
export default async function decorate(block) {
  ...
  await renderPurchaseOrderStatus(block, initialPermissions);

  // React to permission updates
  events.on('auth/permissions', async (updatedPermissions) => {
    await renderPurchaseOrderStatus(block, updatedPermissions);
  });
