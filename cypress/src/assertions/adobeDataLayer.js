 ...
 */
export const expectsEventWithContext = (
  targetEvent,
  targetContexts,
  adobeDataLayer = [],
) => {
  if (targetEvent) {
    const targetEventIndex = adobeDataLayer.findIndex(
      (data) => data?.event === targetEvent,
    );
    expect(targetEventIndex, targetEvent).to.be.greaterThan(-1);
  }
  if (targetContexts?.length) {
    targetContexts.forEach((targetContext) => {
      const contextIndex = adobeDataLayer.findIndex(
        (data) => !!data[targetContext],
      );
